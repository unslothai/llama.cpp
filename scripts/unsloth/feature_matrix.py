...
def probe_mtmd(check: dict, b: Path, gpu: bool) -> str:
    ...
    # the test walks the whole enum, so it proves the table is sound; that this projector
    # is IN the enum is pin_contract.py's job
...
def main() -> int:
    ...
    for name, feat in sorted(doc["features"].items()):
        ...
        if entry["problems"]:
            ...
            # nothing shown either way: not a failure, but not an ok line either
    ...
    if failed:
        ...
        return 1
    # say what was NOT proven alongside what was, or green starts to read as covered
