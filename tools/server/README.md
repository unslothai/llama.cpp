...
| `--preempt, --no-preempt` | with a unified KV cache and more than one slot, park a running request and recompute it later instead of failing every request when the cache fills (default: enabled)<br/>(env: LLAMA_ARG_PREEMPT) |
...
| `llamacpp:n_resume_total` | Counter | Parked slots sent back through prompt processing. |
