...
static bool ggml_backend_cpy_tensor_async_impl(ggml_backend_t backend_src, ggml_backend_t backend_dst, const struct ggml_tensor * src, struct ggml_tensor * dst) {
    if (backend_dst != NULL && backend_dst->iface.cpy_tensor_async != NULL &&
        backend_dst->iface.cpy_tensor_async(backend_src, backend_dst, src, dst)) {
        return true;
    }
    if (backend_src != NULL && backend_src != backend_dst && backend_src->iface.cpy_tensor_async != NULL &&
        backend_src->iface.cpy_tensor_async(backend_src, backend_dst, src, dst)) {
        return true;
    }
    return false;
}
