...
// The three narrow IQ1 loaders below are ggml_cuda_mmq_load_tiles_iq1_s with a different index
// reassembly and a different home for the scale/sign nibble. The tile layout, the 4-bit grid
// unpacking and the (d1q, d1q*delta) pair written into x_ds are all unchanged, so they reuse
// ggml_cuda_mmq_vec_dot_q8_1_q8_1_* just like IQ1_S does.
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps * nrows) {
        ...
        const int qh  = bxi->qh[kqsx];
        const int nib = (bxi->sc[kqsx/2] >> (4*(kqsx%2))) & 0x0F;
    ...
    #pragma unroll
        ...
        const float  d1q   = __half2float(bxi->d) * (2*(nib & 0x07) + 1);
